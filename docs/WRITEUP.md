...
- **Hybrid agent + workflow architecture**: The current system is a pure agent — Claude decides everything in a tool loop. This is great for open-ended Q&A, but some flows (especially callback booking) are deterministic sequences that I enforce through system prompt instructions and backend validation. Per Anthropic's [Building Effective Agents](https://www.anthropic.com/engineering/building-effective-agents) guidance, the right model is a hybrid: agent for open-ended conversation, code-controlled workflows for deterministic sub-tasks. The callback flow would become a state machine — code controls the check → present → confirm → book sequence instead of hoping the LLM follows it. This cuts the booking flow from ~5 LLM rounds to ~2, eliminates the ordering guardrails, and makes the critical path unit-testable. The agent loop stays for KB search and general conversation. See [ADR-007](ADR/007-hybrid-agent-workflow.md) for the full design.
